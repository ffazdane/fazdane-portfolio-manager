...
def delete_active_trades_by_account(account: str) -> int:
    """
    Delete ALL active trades and their legs for the given account number.

    Used by the portfolio position-import flow where a full broker
    position snapshot replaces whatever was previously stored for that account.
    ...
    with get_db() as conn:
        ...
        # Cascade: delete alerts, snapshots, journal, legs, then trades
